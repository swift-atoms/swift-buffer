...
extension `Buffer Storage Slice Capacity Tests` {
    ...
    struct Unit {
        ...
        // MARK: - Buffer.Storage — negative: capacity is fixed at construction,
        // not silently re-derived on every read

        @Test
        func `Buffer Storage capacity is fixed at construction and does not track live base mutation`() {
            var column = FixedColumn(capacity: Index<Int>.Count(UInt(2)))
            column.initialize(at: Index<Int>(_unchecked: Ordinal(UInt(0))), to: 10)
            let storage = Buffer<Never>.Storage(base: column)
            #expect(storage.capacity.count == storage.base.capacity)
        }
